...
class SizesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create size" do
    assert_difference('Size.count') do
      post sizes_url, params: { size: { height: @size.height, length: @size.length, width: @size.width } }
    end
  ...
  test "should update size" do
    patch size_url(@size), params: { size: { height: @size.height, length: @size.length, width: @size.width } }
